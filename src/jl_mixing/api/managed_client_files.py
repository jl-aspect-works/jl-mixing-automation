...
def execute_import(request: ImportRequest) -> tuple[dict[str, Any], int]:
    ...
    try:
        ...
        log_info("import_replan_complete", operation=operation, duration_ms=int((time.monotonic() - plan_started) * 1000), file_count=len(full_plan.get("files", [])))
        ...
        streaming = request.progress == _PROGRESS_MODE
        log_info("progress_mode", operation=operation, streaming=streaming, selected_count=len(plan.get("files", [])))
        progress_callback = (lambda event: _emit_progress(operation, event)) if streaming else None
        result = execute_plan(root, plan, request.decisions or {}, progress=progress_callback)
        log_info("operation_complete", operation=operation, duration_ms=int((time.monotonic() - started) * 1000), selected_count=len(plan.get("files", [])))
