"""Machine-facing plan/execute contract for permanent project deletion."""
...
from ..project_delete import PartialDeletionError, execute_project_delete, plan_project_delete
...
def execute(request: ProjectDeleteRequest, *, plan_only: bool) -> tuple[dict[str, Any], int]:
    operation = "project.delete.plan" if plan_only else "project.delete.execute"
    try:
        if plan_only:
            plan = plan_project_delete(request.workspace, request.client_id, request.project_id)
        else:
            plan = execute_project_delete(request.workspace, request.client_id, request.project_id,
                fingerprint=request.fingerprint or "", confirmed_name=request.confirmed_name or "")
        data = {"summary": plan.summary(), "deleted": not plan_only,
                "external_listening_copies": "retained", "recoverable": False}
        ...
        return _error(operation, "PARTIAL_CLEANUP", str(exc), 1, status="error", data={
            ...
            "external_listening_copies": "retained", "recoverable": False}), 1
    except OSError as exc:
        return _error(operation, "FILESYSTEM_ERROR", str(exc), 1, status="error"), 1


def parse_args(args: list[str], *, plan_only: bool) -> ProjectDeleteRequest:
    options = {"--workspace": "workspace", "--client-id": "client_id", "--project-id": "project_id",
    ...
    while index < len(args):
        ...
        if flag == "--json":
            ...
            raise ArgumentError(f"Unknown project deletion option: {flag}")
        index += 1
    if json_seen != 1 or not all(values.get(key) for key in ("workspace", "client_id", "project_id")):
        raise ArgumentError("Project deletion requires --json, --workspace, --client-id, and --project-id.")
    if plan_only and ("fingerprint" in values or "confirmed_name" in values):
        raise ArgumentError("Project deletion planning does not accept execution confirmation.")
    if not plan_only and not all(values.get(key) for key in ("fingerprint", "confirmed_name")):
        raise ArgumentError("Project deletion execution requires --fingerprint and --confirm-name.")
    return ProjectDeleteRequest(**values)
