...
Download the `v2.3.0` release archive for your platform and verify its accompanying SHA-256 checksum before installing.

- Windows: extract `jl-mixing-2.3.0-windows.zip`, then run `.\windows\install.ps1` in PowerShell. If the script is blocked, run `Unblock-File .\windows\install.ps1` first. The package contains a private Python runtime.
